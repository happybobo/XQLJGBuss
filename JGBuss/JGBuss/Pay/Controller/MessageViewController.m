...
@interface MessageViewController ()<UITableViewDataSource,UITableViewDelegate,BCKeyBoardDelegate,UIScrollViewDelegate>
{

    BOOL isKeyboardDisappear;
}
@property (nonatomic,strong) UITableView *tableView;
@property (nonatomic,strong) JGMessage *lastMessage;
...
-(void)getMessagesFromServer
{
    [self.conversation queryMessagesFromServerWithLimit:20 callback:^(NSArray *objects, NSError *error) {
        ...
        if (objects.count) {
//            for (JGMessage *message in objects) {
...
-(void)didSendText:(NSString *)text
{
    ...
    [attributes setObject:self.conversation.creator forKey:@"touserid"];
    [attributes setObject:USER.name forKey:@"nickname"];
    [attributes setObject:USER.name_image forKey:@"avatar"];
