...
+(NSString *)getMessageTime:(NSTimeInterval)timeStamp
{
    ...
    NSDate *nowDate = [self getDateByTimeStamp:timeNow];
    if (timeNow-3600*24*7>=timeStamp) {//已经超过一个星期了,需要显示具体日期
        ...
        if ([[[NSString stringWithFormat:@"%@",date] substringToIndex:10] isEqualToString:[[NSString stringWithFormat:@"%@",nowDate] substringToIndex:10]]) {
            return timeStr;
        }else{
...
+(NSString *)getWorkTimeStr:(NSString *)startTime
{
